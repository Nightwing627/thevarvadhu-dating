...
<script>
    ...
    var rem_interests = parseInt("<?php echo $this->Crud_model->get_type_name_by_id('member', $this->session->userdata('member_id'), 'express_interest')?>");
    function confirm_interest(id) {
        ...
        if (isloggedin == "") {
            ...
            if (rem_interests <= 0) {
                ...
                $("#modal_buttons").html("<button type='button' class='btn btn-danger btn-sm btn-shadow' data-dismiss='modal' style='width:25%'><?php echo translate('close')?></button> <a href='<?php echo base_url()?>home/plans' class='btn btn-sm btn-base-1 btn-shadow' style='width:25%'><?php echo translate('premium_plans')?></a>");
            }
            else {
                $("#active_modal").modal("toggle");
    ...
    function do_interest(id) {
        ...
        if (isloggedin == "") {
            ...
            setTimeout(function() {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url()?>home/add_interest/"+id,
                    cache: false,
                    success: function(response) {
                        $("#active_modal .close").click();
                        $("#interest_a_"+id).attr("class", "btn btn-dark btn-sm btn-icon-only btn-shadow");
                        $("#interest_a_"+id).attr("title", "<?php echo translate('interest_expressed')?>");
                        $("#interest_a_"+id).attr("data-original-title", "<?php echo translate('interest_expressed')?>");
    ...
    function confirm_remove(id) {
        ...
        if (isloggedin == "") {
            ...
            $("#active_modal").modal("toggle");
            $("#modal_header").html("<?php echo translate('confirm_remove')?>");
    ...
    function remove_shortlist(id) {
        if (isloggedin == "") {
            ...
            setTimeout(function() {
                $.ajax({
                    type: "POST",
                    url: "<?php echo base_url()?>home/remove_shortlist/"+id,
                    cache: false,
                    success: function(response) {
                        $("#active_modal .close").click();
                        $("#member_"+id).hide();
