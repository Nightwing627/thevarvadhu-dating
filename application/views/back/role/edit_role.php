        		...
        		?>
		    		<form class="form-horizontal" id="manage_details_form" method="POST" action="<?=base_url()?>admin/role/update/<?=$row['role_id']?>">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" value="<?php if(!empty($form_contents)){echo $form_contents['name'];}else{  echo $row['name'];}?>" name="name" placeholder="<?= translate('role_name')?>" >
							</div>
						</div>
						<div class="form-group">
							<label class="col-sm-3 control-label" for="description"><b><?= translate('description')?> </span></b></label>
							<div class="col-sm-8">
								<textarea name="description" class="form-control" placeholder="<?php echo translate('role_description'); ?>"><?php if(!empty($form_contents)){echo $form_contents['description'];}else{  echo $row['description'];}?></textarea>
