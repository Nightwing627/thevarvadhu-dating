...
class Instamojo {
    ...
    */
    public function __construct($api_key, $auth_token=null, $endpoint=null)
    {
        $CI=& get_instance();
        $CI->load->database();
        $api_key = $CI->db->get_where('business_settings',array('type'=>'instamojo_api_key'))->row()->value;
        $auth_token = $CI->db->get_where('business_settings',array('type'=>'instamojo_auth_token'))->row()->value;
